...
#define MIN_FIRMWARE_DATE   20260531        //SlotAdapterのFirmware要求Version
...
typedef struct {
    ...
    DWORD mapperAddress;            // use mapper select address
...
// ============================================================================
static BOOL GetFirmwareVersionDate(HANDLE hSerial, unsigned long long* outFirmwareDateTime)
{
    char response[1024];
    int year = 0, day = 0;
    char monthStr[16] = { 0 };
    unsigned long long yyyymmddhhmm = 0;

    if (!outFirmwareDateTime)
        return FALSE;

    *outFirmwareDateTime = 0;

    if (!SendCommand(hSerial, "HVER"))
        return FALSE;
    ...
    // --------------------------------------------------------------------
    {
        ...
        if (pos)
        {
            ...
            if (sscanf_s(pos, "%31[0-9]", fwver, (unsigned)_countof(fwver)) == 1)
            {
                if (strlen(fwver) == 12)
                {
                    yyyymmddhhmm = _strtoui64(fwver, NULL, 10);
                    *outFirmwareDateTime = yyyymmddhhmm;
                    return TRUE;
                }
            }
        }
    ...
    // --------------------------------------------------------------------
    {
        ...
        if (pos)
        {
            ...
            if (sscanf_s(pos, "%15s %d %d",
                monthStr, (unsigned)_countof(monthStr),
                &day, &year) == 3)
            {
                int month = 0;

                if (_stricmp(monthStr, "Jan") == 0) month = 1;
                else if (_stricmp(monthStr, "Feb") == 0) month = 2;
                else if (_stricmp(monthStr, "Mar") == 0) month = 3;
                else if (_stricmp(monthStr, "Apr") == 0) month = 4;
                else if (_stricmp(monthStr, "May") == 0) month = 5;
                else if (_stricmp(monthStr, "Jun") == 0) month = 6;
                else if (_stricmp(monthStr, "Jul") == 0) month = 7;
                else if (_stricmp(monthStr, "Aug") == 0) month = 8;
                else if (_stricmp(monthStr, "Sep") == 0) month = 9;
                else if (_stricmp(monthStr, "Oct") == 0) month = 10;
                else if (_stricmp(monthStr, "Nov") == 0) month = 11;
                else if (_stricmp(monthStr, "Dec") == 0) month = 12;
                else return FALSE;

                yyyymmddhhmm =
                    ((unsigned long long)year * 100000000ULL) +
                    ((unsigned long long)month * 1000000ULL) +
                    ((unsigned long long)day * 10000ULL);
...
static BOOL FirmwareVersionCheck(HANDLE hSerial)
{
    unsigned long long fwDateTime = 0;
    unsigned long long fwDate = 0;
    
    if (!GetFirmwareVersionDate(hSerial, &fwDateTime))
    {
        printf("Failed to get firmware version date\n");
        return FALSE;
    }
    
    fwDate = fwDateTime / 10000ULL;
    
    if (fwDate < MIN_FIRMWARE_DATE)
    {
        printf("Firmware version is too old: %llu (required: %d or later)\n", fwDate, MIN_FIRMWARE_DATE);
        return FALSE;
    }
    
    printf("Firmware version check... OK\n");
    return TRUE;
}
...
static BOOL setMegaROMconfig(HANDLE hSerial, DWORD selecterAddress, DWORD readAddress, DWORD bankSize)
{
    char command[128];
    char response[MAX_RESPONSE_LEN];

    sprintf_s(command, sizeof(command), "RMSET,%04lX,%04lX,%04lX", selecterAddress, readAddress, bankSize);
...
static BOOL slotMegaROMdump(HANDLE hSerial, DWORD bankSize, DWORD startBank, DWORD endBank, BYTE* data)
{
    char command[128];
    char response[MAX_RESPONSE_LEN];
    DWORD bank;

    sprintf_s(command, sizeof(command), "RMRD,%X,%X", startBank, endBank);
    ...
    for (bank = startBank; bank < endBank; bank++) {
        printf("Saved bank %lu (0x%04lX - 0x%04lX)\n", bank, bank * bankSize, bankSize * (bank + 1)-1);
        if (!RecvBinaryBlock(hSerial, data+ (bankSize * bank), bankSize))
            return FALSE;
    }
...
static BOOL SweepAddress0AndCheck(HANDLE hSerial)
{
    ...
    for (d = 100; d <= 1000; d += 100)
    {
        ...
        if (!hardwareSetting(hSerial, 0, d))  return FALSE;
        if (!hardwareSetting(hSerial, 1, d + 100))  return FALSE;
...
static BOOL CheckHashWithRetry(HANDLE hSerial)
{
    ...
    if (!hardwareSetting(hSerial, 1, 100))  return FALSE;
    ...
    printf("FAILED\n");
    printf("Checking Read setting ... \n");
    // 1us 刻みで 100us まで変更しながら確認
    if (SweepAddress0AndCheck(hSerial))  return TRUE;
...
static BOOL DetectASCII8K(HANDLE hSerial, ROM_INFO* romInfo)
{
    ...
    BYTE sramData[4];
    const DWORD filledFFHash = HashFilledFF(HASH_SIZE);
    static const DWORD bankSteps[] = { 0, 4, 8, 16, 32, 64, 128, 256 };
    DWORD stepIndex;
    ...
    // hashが同一値の場合は除外
    if ((hashA[1] == hashA[0]) && (hashA[2] == hashA[0]) && (hashA[3] == hashA[0])) return FALSE;
    ...
    for (stepIndex = 0; stepIndex < sizeof(bankSteps) / sizeof(bankSteps[0]); stepIndex++)
    {
        bankNum = bankSteps[stepIndex];

        if (!slotWrite(hSerial, 0x6000, (BYTE)(bankNum + 3))) return FALSE;
        if (!slotWrite(hSerial, 0x6800, (BYTE)(bankNum + 2))) return FALSE;
        if (!slotWrite(hSerial, 0x7000, (BYTE)(bankNum + 1))) return FALSE;
        if (!slotWrite(hSerial, 0x7800, (BYTE)(bankNum + 0))) return FALSE;
        ...
        if (!slotWrite(hSerial, 0xA000, (BYTE)~sramOrgData[0])) return FALSE;
        if (!slotWrite(hSerial, 0xB000, (BYTE)~sramOrgData[2])) return FALSE;
...
static BOOL DetectASCII16K(HANDLE hSerial, ROM_INFO* romInfo)
{
    ...
    BYTE sramData[4];
    const DWORD filledFFHash = HashFilledFF(HASH_SIZE);
    static const DWORD bankSteps[] = { 0, 2, 4, 8, 16, 32, 64, 128 };
    DWORD stepIndex;
    ...
    if ((hashA[1] == hashA[0]) && (hashA[2] == hashA[0]) && (hashA[3] == hashA[0])) return FALSE;
    // 16k mirror ROMも除外
    if ((hashA[2] == hashA[0]) && (hashA[3] == hashA[1])) return FALSE;
    ...
    for (stepIndex = 0; stepIndex < sizeof(bankSteps) / sizeof(bankSteps[0]); stepIndex++)
    {
        ...
        if (!slotWrite(hSerial, 0x7000, (BYTE)(bankNum + 0))) return FALSE;
        ...
        if (!slotWrite(hSerial, 0x8000, (BYTE)~sramOrgData[0])) return FALSE;
        if (!slotWrite(hSerial, 0xA000, (BYTE)~sramOrgData[2])) return FALSE;
...
static BOOL DetectKONAMI8K(HANDLE hSerial, ROM_INFO* romInfo)
{
    ...
    BOOL foundPattern = FALSE;
    const DWORD filledFFHash = HashFilledFF(HASH_SIZE);
    static const DWORD bankSteps[] = { 0, 4, 8, 16, 32 };
    DWORD stepIndex;
    ...
    for (stepIndex = 0; stepIndex < sizeof(bankSteps) / sizeof(bankSteps[0]); stepIndex++)
    {
        ...
        if (!slotWrite(hSerial, 0x6000, (BYTE)(bankNum + 2))) return FALSE;
        if (!slotWrite(hSerial, 0x8000, (BYTE)(bankNum + 0))) return FALSE;
        if (!slotWrite(hSerial, 0xA000, (BYTE)(bankNum + 1))) return FALSE;
        ...
        // Patternが一致の場合
        if ((hashB[0] == hashA[0]) && (hashB[1] == hashA[3]) && (hashB[2] == hashA[1]) && (hashB[3] == hashA[2]))
        {
            if (bankNum != 0) break;
            foundPattern = TRUE;
        }

        if ((filledFFHash == hashB[1]) && (filledFFHash == hashB[2]) && (filledFFHash == hashB[3]))
        {
        ...
        if ((bankNum == 0x00) && ((hashA[1] == hashB[1]) || (hashA[2] == hashB[2]) || (hashA[3] == hashB[3])))
        {
            break;
        }
...
static BOOL DetectKONAMI_SCC(HANDLE hSerial, ROM_INFO* romInfo)
{
    ...
    {
        bankNum = bankSteps[stepIndex];
        if (!slotWrite(hSerial, 0x5000, (BYTE)(bankNum + 3))) return FALSE;
        if (!slotWrite(hSerial, 0x7000, (BYTE)(bankNum + 2))) return FALSE;
        if (!slotWrite(hSerial, 0x9000, (BYTE)(bankNum + 1))) return FALSE;
        if (!slotWrite(hSerial, 0xb000, (BYTE)(bankNum + 0))) return FALSE;
        ...
        // Patternが一致の場合
        if ((hashB[3] == hashA[0]) && (hashB[2] == hashA[1]) && (hashB[1] == hashA[2]) && (hashB[0] == hashA[3]))
        {
            if (bankNum != 0) break;
            foundPattern = TRUE;
        }
        ...
        if ((bankNum == 0x00) && ((hashA[0] == hashB[0]) || (hashA[1] == hashB[1]) || (hashA[2] == hashB[2]) || (hashA[3] == hashB[3])))
        {
            break;
        }
...
static BOOL ReadMegaROM(HANDLE hSerial, ROM_INFO* romInfo, BYTE* outData)
{
    printf("=== Reading %s ROM ===\n",romInfo->mapperName);
    if(!setMegaROMconfig(hSerial, romInfo->mapperAddress, romInfo->readAreaStart, romInfo->readAreaSize)) return FALSE;
    if(!slotMegaROMdump(hSerial, romInfo->readBankSize, 0, romInfo->bankCount, outData)) {
        printf("Failed to read bank\n");
        return FALSE;
    }
    printf("\nTotal bytes read: %lu (0x%lX)\n", romInfo->bankCount * romInfo->readBankSize, romInfo->bankCount * romInfo->readBankSize);
    return TRUE;
}
...
static BOOL ReadStandardROM(HANDLE hSerial, ROM_INFO* romInfo, BYTE* outData)
{
    ...
    if (!hardwareSetting(hSerial, 0, 200))  return FALSE;
    if (!hardwareSetting(hSerial, 1, 200 + 100))  return FALSE;
...
int ProcessROMRead(const wchar_t* outputFileArg, bool autoFileNameMode)
{
    ...
    for (int count = 0; SetupDiEnumDeviceInfo(hDevInfo, count, &data); count++)
    {
        ...
        if (!FirmwareVersionCheck(hSerial))
        {
            printf("ERROR: Firmware version check failed\n\n");
            CloseHandle(hSerial);
            continue;
        }
